...
setclipboard('https://dsc.gg/1wx')
-- Function to check if the local player is allowed
local function isPlayerAllowed(player)
    -- Check if the player's ID or username is in the allowed list
    for _, allowed in ipairs(blahskidnig) do
        if player.UserId == allowed or player.Name == allowed then
            return true
        end
    end
    return false
...
-- Function to kick the player if not allowed
local function kickPlayer(player)
    -- Kick the local player with a message
    player:Kick("Unauthorized to use ts vro")
end

-- Check if the local player is allowed
if not isPlayerAllowed(LocalPlayer) then
    -- If not allowed, kick the local player
...
RightGroupBox:AddLabel('Fog Color Picker'):AddColorPicker('FogColorPicker', {
    ...
    -- Configuration Flags
    local Flags = {
    cframeSpeedEnabled = true, -- Initially off
    cframeSpeedToggleAllowed = true, -- Toggle must be enabled via UI
...
TextChatService = game:GetService("TextChatService")
Promo = {
    "NO WAY solaris.pub ON TOP",
    "✅Get solaris.pub today✅",
    "✅solaris.pub #1 paid✅",
    "aint no wai you aint using solaris.pub",
    "✅solaris.pub is 25$ lifetime✅",
    "solaris.pub > any external or script",
    "✅#1 autostomp by solaris.pub✅",
    "✅best anims by solaris.pub✅"
}
    ...
    local function onKeyPress(input, gameProcessed)
        if not gameProcessed then
            if _G.autoStomp and input.KeyCode == stompKey then
                ...
                if isLooping then
                    showNotification("Auto Stomp Enabled", "solaris.pub", 3)
                    startStomping()  -- Now calling startStomping properly
                else
                    showNotification("Auto Stomp Disabled", "solaris.pub", 3)
                    stopStomping()
                end
            end
        end
    ...
    local function teleportAndStomp(targetPlayer)
        if targetPlayer and targetPlayer.Character then
            ...
            if targetHumanoid and torso and playerRoot then
                ...
                showNotification("Stomping", "solaris.pub", 3)
            ...
            showNotification("No Target Found", "solaris.pub", 3)
        end
    end
        ...
        elseif Config.enabled and (Config.spyOnMyself == true or p ~= player) then
            ...
            if hidden and Config.enabled then
                if Config.public then
                    saymsg:FireServer((Config.publicItalics and "/me " or '') .. "{solaris.pub} [" .. p.Name .. "]: " .. msg, "All")
                else
...
WatermarkConnection = game:GetService('RunService').RenderStepped:Connect(function()
    ...
    Library:SetWatermark(('solaris.pub | %s fps | %s ms'):format(
        math.floor(FPS),
        ping
    ))
end)
